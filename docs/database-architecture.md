...
As migrations em `supabase/migrations/` são a especificação executável da primeira etapa e prevalecem sobre a proposta exploratória mais detalhada deste documento. O modelo deliberadamente reduz o número de conceitos: `vinculos_usuario` substitui a separação de papéis e grants; `configuracoes_rede` é o ponteiro da publicação ativa; `eventos_fluxo_ppp` concentra o histórico de transições; `arquivos_ppp` guarda somente metadados de objetos privados; e indicadores, textos, seleções e detalhes do formulário ficam no `answers` JSONB de uma revisão.

O banco inicial contém organizações (`redes`, `superintendencias_regionais`, `escolas`), identidade e escopo (`perfis_usuario`, `vinculos_usuario`), conteúdo e catálogos (`publicacoes_conteudo`, `configuracoes_rede`, `catalogos`, `opcoes_catalogo`) e o domínio documental (`projetos_politico_pedagogicos`, `versoes_ppp`, `revisoes_ppp`, `progresso_ppp`, `participantes_ppp`, `arquivos_ppp`, `eventos_fluxo_ppp`, `eventos_auditoria`). Isso preserva integridade onde há relação entre entidades e evita transformar cada pergunta pedagógica mutável em coluna.
