-- Corrige o escopo de editor de escola e sincroniza a conta Google já criada.
...
begin
  ...
  if found then
    ...
    if not exists (select 1 from public.vinculos_usuarios v where v.usuario_id=usuario.id and v.rede_ensino_id=provisionamento.rede_ensino_id and v.escola_id is not distinct from provisionamento.escola_id and v.papel=provisionamento.papel and v.revogado_em is null) then
...
select app.sincronizar_acesso_usuario(id)
from auth.users
where lower(email) = 'lucaskayke06@gmail.com';
