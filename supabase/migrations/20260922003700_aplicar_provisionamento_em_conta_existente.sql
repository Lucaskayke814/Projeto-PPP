-- Aplica imediatamente o provisionamento quando o e-mail ja possui conta Auth.
begin;

create or replace function public.salvar_provisionamento_acesso(
  email_destino text, papel_destino text, rede_destino_id uuid,
  regional_destino_id uuid default null, escola_destino_id uuid default null, nome_destino text default ''
)
returns uuid
...
begin
  ...
  if papel_destino not in ('editor_escola','leitor_regional','leitor_rede','curador_conteudo','administrador_acessos') then raise exception 'Papel invalido'; end if;
  ...
  if usuario_existente_id is not null then perform app.sincronizar_acesso_usuario(usuario_existente_id); end if;
